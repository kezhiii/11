...
from utils_copy import SamplewiseAccuracy, MeanAccuracy, MeanIoU, FrequencyWeightedIoU, plot_barcode, edit_score, f_score
...
def evaluate_with_metrics(model, dataloader, logger, epoch, device, is_final=False):
    ...
    with torch.no_grad():
        ...
        for i, (data, target) in enumerate(dataloader):
            ...
            # output是一个概率张量第零个维度是批次数量，第一个维度是0和1的概率，哪个概率大输出哪个索引
            prediction = torch.argmax(output, dim=1)
            labels.append(label.cpu().numpy())
            levels.append(level.cpu().numpy())
...
class Trainer:
    ...
    def train(self, train_dataset, val_dataset, test_dataset, loss_weights, soft_label,
...
if __name__ == '__main__':
    ...
    val_dataset = SatellitePoL(data_dir, val_ids, labelfile, selected_features)
