...
def evaluate_with_metrics(model, dataloader, logger, epoch, device, is_final=False):
    ...
    plot_barcode(6, features[:10], predictions[:10], True, save_path)
    ...
    if is_final:
        ...
        logger.info(classification_report(predictions, digits=4))

    return pointacc, meanacc, meaniou, freqiou,
...
class Trainer:
    def __init__(self, encoder_params, decoder_params, diffusion_params,
                 sample_rate, temporal_aug, set_sampling_seed, device):
        ...
        # 实例化 ASDiffusionModel
        num_classes = 6
        self.model = ASDiffusionModel(encoder_params, decoder_params, diffusion_params, num_classes, self.device)
    ...
    def train(self, train_dataset, test_dataset, loss_weights, soft_label,
        ...
        # 三种损失，交叉熵损失（分类任务），二元交叉熵损失（二分类任务），均方差损失（回归任务）
        ...
        for epoch in range(restore_epoch + 1, num_epochs):
            ...
            if epoch % 10 == 0:
                ...
                pointacc_, meanacc_, meaniou_, freqiou_ = evaluate_with_metrics(self.model, test_loader, logger, epoch,
                                                                                self.device)
